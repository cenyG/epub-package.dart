...
class NavLabel {
  ...
  static NavLabel fromXmlElement(XmlElement el) {
    final text = el.findElements('text');
    final label = NavLabel(text.isEmpty ? '' : text.first.text.trim());
...
class NavPoint {
  ...
  String? get label => labels?.firstOrNull?.text;
  ...
  static NavPoint fromXmlElement(String baseDir, XmlElement el) => NavPoint(
      id: el.getAttribute('id'),
      klass: el.getAttribute('class'),
      playOrder: int.parse(el.getAttribute('playOrder') ?? '-1'),
      labels: List.from(
          el.findElements('navLabel').map((el) => NavLabel.fromXmlElement(el))),
      content: pathJoin([
        baseDir,
        el.findElements('content').firstOrNull?.getAttribute('src') ??
            'PARSING_ERROR'
      ]),
...
class EpubNav extends _EpubXmlBase {
  ...
  static String? _getDocText(XmlElement? node) {
    if (node == null) return null;

    final text = node.findElements('text').firstOrNull;
  ...
  static EpubNav? fromNcx(String dirname, String? xmlStr) {
    ...
    // loadDocAttributes
    nav._title = _getDocText(root.findElements('docTitle').firstOrNull);
    nav._authors
        .addAll(root.findElements('docAuthor').map(_getDocText).whereNotNull());

    // loadNavMapList
    final navMapList = root.findElements('navMap').firstOrNull;
    if (navMapList != null) {
